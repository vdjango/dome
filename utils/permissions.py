'''
来源： https://github.com/ShszCraft/lhwl_restful/blob/master/api/permissions.py
工具类：
    封装权限类，具体控制个体用户具体权限
'''
import re
...
from utils.drump_load_token import loads_token
...
class WechatSafe(permissions.BasePermission):
    """
      A base class from which all permission classes should inherit.
      微信端安全验证，包括用户id和浏览器验证
      """
    ...
    def has_permission(self, request, view):
        ...
        # if not re.match('.*Android.*MicroMessenger.*|.*iPhone.*MicroMessenger|.*MicroMessenger', UA):
        #     return False
        if not token:
            return False
        if not userid:
            ...
            userid = eval(userid)
        result = loads_token(token, userid)
        if result:
            return True
        else:
            return False
...
class MethodsPermission(permissions.BasePermission):
    """
    Custom permission to only allow owners of an object to edit it.
    """
    SAFE_METHODS = ('GET', 'HEAD', 'PATCH', 'PUT', 'POST', 'DELETE', 'OPTIONS')
...
class WuheMethodsPermission(permissions.BasePermission):
    """
    Custom permission to only allow owners of an object to edit it.
    """
    SAFE_METHODS = ('GET', 'HEAD', 'PATCH', 'PUT', 'POST', 'DELETE', 'OPTIONS')
...
class CreateRecordPermission(permissions.BasePermission):
    """
    Custom permission to only allow owners of an object to edit it.
    """
    ...
    def test(self, request, view):
        ...
        if user:
            if view.action == "create":
                if user.Status == 0:
                    return False
                if user.Obj1 == "0" and testtype == "0":
                    return False
                if user.Obj2 == "0" and testtype == "1":
                    return False
        else:
            return False
        return True
